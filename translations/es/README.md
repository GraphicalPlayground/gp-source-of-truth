...
Si descubres una vulnerabilidad de seguridad en `{{GP_REPO_NAME}}`, por favor no la reportes abriendo un issue público. En su lugar, consulta nuestra [Política de Seguridad](./SECURITY.md) para obtener instrucciones sobre cómo divulgar la vulnerabilidad de forma segura a los mantenedores.

### Licencia

`{{GP_REPO_NAME}}` es software de código abierto. Consulta el archivo [LICENSE.md](../../LICENSE.md) en el directorio raíz para conocer los términos completos sobre modificación, distribución y uso en tus propios proyectos.

### Donaciones

Si encuentras `{{GP_REPO_NAME}}` útil para tu aprendizaje, investigación académica o desarrollo de videojuegos, considera apoyar el proyecto. ¡Mantener un motor de gráficos moderno en C++ requiere tiempo y recursos significativos!
...
Si tienes preguntas, sugerencias o quieres compartir tus proyectos construidos con `{{GP_REPO_NAME}}`, ¡nos encantaría saber de ti! Puedes contactarnos a través de los siguientes canales:
